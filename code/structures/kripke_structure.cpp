...
: _transitions(std::move(tr)), _aps(std::move(aps)), _state_formula(state_f), _init_formula(init_f), _bdd_var_mapping(bdd_var_mapping), _initial_states(BddUtils::expr_to_bdd(mgr, _init_formula, _bdd_var_mapping)), _ap_to_var_idx(ap_to_var_idx)
{ }
...
const ConcreteSet &KripkeStructure::get_initial_states() {
    return _initial_states;
}

const std::map<z3::expr, size_t, Z3ExprComp> &KripkeStructure::get_bdd_var_mapping() const {
    return _bdd_var_mapping;
}
